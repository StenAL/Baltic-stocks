...
public class StockDataImportingService {
        ...
        try {
            ...
            if (financialDataTable == null) {
                throw new RuntimeException(
                        "Could not fetch stock: no matching CSS selector for '#OverviewFinancials table'");
            }
            ...
            if (keyStatsTable == null) {
                throw new RuntimeException(
                        "Could not fetch stock: no matching CSS selector for '#OverviewRatios table'");
            }
            ...
            if (dividendsTable == null) {
                throw new RuntimeException(
                        "Could not fetch stock: no matching CSS selector for '#OverviewDividends table'");
            }
    ...
    private String generateApiUrl(String isin) {
        String exchangeCode =
                Exchange.findByPrefix(isin.substring(0, ISIN_PREFIX_LENGTH)).getExchangeCode();
        return API_BASE_URL + "?externalid="
                + isin + "&externalidexchange="
                + exchangeCode
    ...
    private KeyStats convertHtmlToKeyStats(Element keyStatsTable) {
        ...
        return KeyStats.builder()
                .priceEarningTtm(
                        StringParserUtil.parseDoubleIfPresent(squares.get(1).text())
                                .orElse(null))
                .priceBook(StringParserUtil.parseDoubleIfPresent(squares.get(2).text())
                        .orElse(null))
                .priceSalesTtm(
                        StringParserUtil.parseDoubleIfPresent(squares.get(3).text())
                                .orElse(null))
                .revenueGrowthThreeYearAvg(
                        StringParserUtil.parseDoubleIfPresent(squares.get(4).text())
                                .orElse(null))
                .epsGrowthThreeYearAvg(
                        StringParserUtil.parseDoubleIfPresent(squares.get(5).text())
                                .orElse(null))
                .operatingMarginTtm(
                        StringParserUtil.parseDoubleIfPresent(squares.get(6).text())
                                .orElse(null))
                .netMarginTtm(
                        StringParserUtil.parseDoubleIfPresent(squares.get(7).text())
                                .orElse(null))
                .roeTtm(StringParserUtil.parseDoubleIfPresent(squares.get(8).text())
                        .orElse(null))
                .debtEquity(StringParserUtil.parseDoubleIfPresent(squares.get(9).text())
                        .orElse(null))
                .build();
    }
    ...
    private List<Dividend> convertHtmlToDividends(Element dividendsTable) {
        ...
        return List.of(
                Dividend.builder()
                        .declaredDate(StringParserUtil.parseLocalDateIfPresent(
                                        squares.get(1).text(), DATE_TIME_FORMATTER)
                                .orElse(null))
                        .exDiv(StringParserUtil.parseLocalDateIfPresent(
                                        squares.get(3).text(), DATE_TIME_FORMATTER)
                                .orElse(null))
                        .paid(StringParserUtil.parseLocalDateIfPresent(
                                        squares.get(5).text(), DATE_TIME_FORMATTER)
                                .orElse(null))
                        .amount(StringParserUtil.parseDoubleIfPresent(
                                        squares.get(7).text())
                                .orElse(null))
                        .build(),
                Dividend.builder()
                        .declaredDate(StringParserUtil.parseLocalDateIfPresent(
                                        squares.get(2).text(), DATE_TIME_FORMATTER)
                                .orElse(null))
                        .exDiv(StringParserUtil.parseLocalDateIfPresent(
                                        squares.get(4).text(), DATE_TIME_FORMATTER)
                                .orElse(null))
                        .paid(StringParserUtil.parseLocalDateIfPresent(
                                        squares.get(6).text(), DATE_TIME_FORMATTER)
                                .orElse(null))
                        .amount(StringParserUtil.parseDoubleIfPresent(
                                        squares.get(8).text())
                                .orElse(null))
