...
public class DataImportingJob implements ApplicationListener<ApplicationReadyEvent> {
    ...
    private static final List<ImportingRequest> BALTIC_MAIN_LIST_DATA = List.of(
            ...
            new ImportingRequest("LT0000102253", "ROE1L", "Artea Bankas"),
    ...
    public void onApplicationEvent(final ApplicationReadyEvent event) {
        LocalDateTime lastFetch =
                batchRepository.findTopByOrderByTimestampDesc().getTimestamp();
        if (lastFetch.isBefore(LocalDateTime.now().minusDays(DataImportingJob.FETCH_FREQUENCY_DAYS))) {
            log.info(
                    "Current stock data is older than {} days. Fetching new data.",
                    DataImportingJob.FETCH_FREQUENCY_DAYS);
            try {
                updateAllStocks();
            } catch (Exception e) {
    ...
    public void updateAllStocks() {
        ...
        Batch batch = Batch.builder().timestamp(LocalDateTime.now()).build();
